...
from codr.application.entities import User, VersionControlType
from codr.application.interactors.github.create_access_token import (
    CreateAccessToken,
    CreateAccessTokenRequest,
)
from codr.application.interactors.github.refresh_access_token import (
    RefreshAccessToken,
...
class AuthenticateUser:
    ...
    def execute(self, request: AuthenticateUserRequest) -> AuthenticateUserResponse:
        ...
        if not access_token:
            access_token = self.__create_access_token.execute(
                CreateAccessTokenRequest(code="", user_id=request.user_id)
            ).user.get_access_token(request.version_control_type)
        else:
            # Refresh access token if it is expired
            if not user.has_valid_access_token(request.version_control_type):
                access_token = self.__refresh_access_token.execute(
                    RefreshAccessTokenRequest(
