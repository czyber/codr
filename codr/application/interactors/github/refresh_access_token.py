...
from codr.application.entities import User, VersionControlType, SetTokenRequest
...
from codr.utils import Id, GitHubCredentials
...
class RefreshAccessToken:
    ...
    def execute(self, request: RefreshAccessTokenRequest) -> RefreshAccessTokenResponse:
        ...
        if not refresh_token:
            raise ValueError("User does not have a refresh token for the specified version control type")
        ...
        if "error" in response_data:
            raise ValueError(f"Error while refreshing access token for GitHub: {response_data.get('error_description')}")
        access_token = response_data.get("access_token")
        refresh_token = response_data.get("refresh_token")
        expires_in = response_data.get("expires_in")
        refresh_token_expires_in = response_data.get("refresh_token_expires_in")

        user.set_access_token(SetTokenRequest(
