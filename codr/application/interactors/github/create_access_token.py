...
class CreateAccessToken:
    ...
    def execute(self, request: CreateAccessTokenRequest) -> CreateAccessTokenResponse:
        ...
        if "error" in response_data:
            raise ValueError(
                f"Error while creating access token for GitHub: {response_data.get('error_description')}"
            )
        ...
        refresh_token_expires_at = datetime.now() + timedelta(
            seconds=response_data.get("refresh_token_expires_in")
        )
        user = self.__get_user.execute(GetUserRequest(id=request.user_id)).user
        user.version_control_infos.append(
            VersionControlInfo.create_github(
